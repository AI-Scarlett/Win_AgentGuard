...
public sealed class WindowsToastNotificationService : INotificationService
{
    ...
    // PowerShell toast script. The {0} placeholder is the toast XML and
    // {1} is the toast app id. PowerShell @'...'@ here-string closing
    // marker must start at column 0, so this lives as a class-level
    // verbatim string (no leading whitespace to be stripped) and uses
    // string.Format for the dynamic substitutions.
    private const string ToastPowerShellTemplate = @"
...
'@)
  ...
  [Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier('{1}').Show($toast)
} catch {
...
";
    ...
    private static string BuildPowerShellToastScript(string xml)
    {
        ...
        return string.Format(ToastPowerShellTemplate, xml, ToastAppId);
    }
}
