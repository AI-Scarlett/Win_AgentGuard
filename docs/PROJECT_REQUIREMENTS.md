...
8. 用户可查看 Agent Monitor 概览，合并实时会话、历史会话、token、context、rate limit、工具调用、命令调用和文件访问。
...
- Monitor Overview：对齐 macOS 版近期 abtop / TokenScope 方向，展示本机 agent 会话、token、context、rate limit、工具、命令和文件访问汇总。
- Audit：展示最近本地审计记录。
...
主应用壳层需要跟进 macOS 版 2026-06-12 UI/UX 重构：

- 使用左侧命令中心导航，不继续使用顶部默认系统 tab 外观。
- 品牌区展示 AgentGuard 图标、产品名和实时运行状态。
- 导航项使用固定图标槽、选中强调轨、hover 表面和统一的状态化交互。
- 页面、卡片、表格、输入框和按钮必须统一使用主题资源，不能散落硬编码白色或灰色。
- 保持紧凑的操作台信息密度，不做营销式首页。

Settings 必须提供以下外观设置：

- Color Theme：`porcelain`、`aurora`、`rose`、`shield`。
- Appearance Mode：跟随系统、浅色、深色。
- 主题切换需要即时应用并持久化到 `%APPDATA%\AgentGuard\settings.json`。
- 跟随系统模式需要响应 Windows 系统主题变化。
...
### 8. Agent Monitor / TokenScope 概览

Windows 版需要跟进 MacCleaner 近期 AgentGuard 功能迭代：

- 保持 Approvals 作为第一个 tab，不能把审批链路藏到历史或设置页。
- 非审批 hook 事件必须进入 Audit，不能出现“审批/审计页没有数据但 agent 实际有动作”的断链。
- Agent Monitor Overview 必须按 abtop 的产品语义展示“本机正在发生什么”，不是只显示一个 token 图表：
  - 会话身份：Agent、Session ID、项目、工作目录、模型。
  - 当前状态：live hook 会话阶段、最后活动时间。
  - 操作量：工具调用、命令调用、文件访问。
  - TokenScope：从 Codex `event_msg` / `token_count` 解析 input、output、cache、reasoning、total token。
  - Context：使用 `model_context_window` 计算上下文占用比例。
  - Rate limit：解析 5h / 7d 使用比例和计划类型。
- Token 统计必须优先使用 `total_token_usage` 的累计差值，避免重复累加同一 session 的累计值；缺少累计值或累计重置时才回退到 `last_token_usage`。
- 历史扫描结果必须写入 `%APPDATA%\AgentGuard\agent-history-cache.json`，启动时可加载缓存，让 Overview 不因为未手动扫描而总是空白。
...
- WPF 主界面和 8 个功能 tab。
- Agent Monitor Overview tab，保留 Approvals 第一优先级。
- 左侧命令中心导航、统一主题化控件和品牌状态头。
- Default White、Aurora Cyan、Rose Signal、Blue Shield 四套主题。
- 跟随系统、浅色、深色三种外观模式。
...
- AgentSessionScanner 对 Codex `token_count`、context window、rate limit 的解析。
- AgentMonitorOverviewService 聚合 live session、历史记录、token usage 和审计记录。
- 历史扫描缓存启动加载。
- JSON 本地持久化。
